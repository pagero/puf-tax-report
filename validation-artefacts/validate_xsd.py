...
XSD = XSD_DIR / "Pagero-Universal-Format-Tax-Report.xsd"  # Default schema (will auto-detect)
...
def validate_xml_file(xml_path: Path, schema: etree.XMLSchema) -> tuple[bool, list[str], str]:
    ...
    try:
        ...
        if root_element == "TaxReport":
            schema = schema
            schema_name = "TaxReport"
        else:
...
def main():
    ...
    if not XSD.exists():
        print(f"[ERROR] Tax Report XSD schema not found at: {XSD}")
        sys.exit(1)
    ...
    try:
        ...
        tax_report_schema_doc = etree.parse(str(XSD))
        tax_report_schema = etree.XMLSchema(tax_report_schema_doc)
        print(f"[OK] Tax Report schema loaded successfully")
    except Exception as e:
    ...
    invalid_count = 0
    error_details = []
    schema_stats = {"TaxReport": 0, "Unknown": 0}
    ...
    for xml_file in xml_files:
        ...
        is_valid, errors, schema_used = validate_xml_file(xml_file, tax_report_schema)
    ...
    print(f"\nBy Schema:")
    print(f"  TaxReport:       {schema_stats.get('TaxReport', 0)}")
